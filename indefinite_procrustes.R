...
f <- function(U,X,Y,A = NULL,b = NULL, gamma = NULL,grad = FALSE) {
  ...
  if (!grad) {
    ...
    grad_val <- 2*(A %*% U - b)
    return( convertToVector(grad_val
    ))
  }
...
constraint <- function(U,G,grad = FALSE) {
  ...
  if (!grad) {
    ...
    return(convertToVector(t(U) %*% G %*% U - G))
    
  } else {
    #cons <- convertToVector(t(U) %*% G %*% U)
    grad_cons <- NULL
    k <- 1
    for (i in c(1:dim(U)[1])) {
      for (j in c(1:dim(U)[2])) {
        ...
        grad_cons <- rbind(grad_cons,convertToVector(G %*% U %*%  J))
        k <- k +1
      }
    }
...
solve_procrustes_problem <- function(X,Y,p,q,I0 = NULL) {
  ...
  if (is.null(I0)) {
    I0 <- convertToVector(sparseMatrix(i = c(1:dim(G)[1]),j=c(1:dim(G)[1]),x=1))
  } else {
    I0 <- convertToVector(t(I0))
  }
  
  control.outer <- list(itmax = 100)#,method = "nlminb")
  result <- alabama::auglag(par = I0,fn = f_new,gr = f_grad,heq = heq
                            , heq.jac = heq_jac,control.outer = control.outer)
  #result <- alabama::constrOptim.nl(par = I0,fn = f_new,gr = f_grad,heq = heq
  #                       , heq.jac = heq_jac,control.outer = control.outer)
  return(result) 
